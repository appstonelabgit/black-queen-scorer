...
void _applyRound(
  ...
  // Free-score round: the per-player number entered is the delta, verbatim.
  // Bid fields (team/bidAmount/won) and the caller bonus don't apply.
  final freeScores = round.scores;
  if (freeScores != null) {
    for (final p in players) {
      acc[p] = (acc[p] ?? 0) + (freeScores[p] ?? 0);
    }
    return;
  }
...
SessionStats computeStats(Session session) {
  ...
  if (session.rounds.isNotEmpty) {
    ...
    for (final r in session.rounds) {
      // "Winning side" is a bid concept; free rounds have no sides.
      if (r.isFree) continue;
