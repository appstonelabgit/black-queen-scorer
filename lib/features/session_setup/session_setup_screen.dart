  ...
  Future<void> _start() async {
    ...
    final session = Session.create(
      ...
      settings: SessionSettings(
        // Free-score sessions never apply the caller bonus.
        bonusEnabled: _isFree ? false : _bonusEnabled,
            ...
            Expanded(
              child: ListView(
                ...
                children: [
                  ...
                  // Bonus only applies to bid scoring; free-score has no caller.
                  if (!_isFree) ...[
                    _BonusSection(
...
/// Scoring-mode picker. Bid = the proven caller/contract engine; Free-Score =
/// type any number per player per round (Rummy, Hearts, Marriage, …).
class _ModeSection extends StatelessWidget {
  final ScoreMode mode;
  final ValueChanged<ScoreMode> onChanged;
  ...
  Widget build(BuildContext context) {
    final scheme = Theme.of(context).colorScheme;
    final text = Theme.of(context).textTheme;
    final isFree = mode == ScoreMode.free;
    return _Section(
      icon: PhosphorIconsRegular.listNumbers,
      title: 'Scoring mode',
      subtitle: isFree
          ? 'Type any points per player each round.'
          : 'Caller bids a target; team wins or loses it.',
      child: SegmentedButton<ScoreMode>(
        segments: const [
          ButtonSegment(
            value: ScoreMode.bid,
            label: Text('Bid'),
            icon: Icon(PhosphorIconsRegular.crown, size: 16),
          ),
          ButtonSegment(
            value: ScoreMode.free,
            label: Text('Free-Score'),
                ...
                ? Padding(
                    ...
                    child: Column(
                      ...
                      children: [
                        Text(
                          isFree
                              ? 'Each round, type every player\'s points for that hand. Totals add up across rounds. Use − for penalties.\n\nWorks for Rummy, Hearts, Marriage, and any point-per-round game.'
                        ...
                        Container(
                          ...
                          child: Text(
                            isFree
                                ? 'Round 1   A: 12   B: 0   C: 8   D: −4\n'
                                    'Round 2   A: 5    B: 20  C: 0   D: 11\n'
                                    'Totals    A: 17   B: 20  C: 8   D: 7'
